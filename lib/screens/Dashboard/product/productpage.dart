...
class _ViewProductState extends State<ViewProduct> {
  ...
  void initState() {
    ...
    service.getvechiledetailsbyuserid(token: widget.token,
        id: widget.userId.toString())?.then((value) {
          if(value?.body.length != 0){
            categoryController.selectedVariantId.value = value!.body[0].variantId.toString();
            categoryController.selectedVariantName.value = value.body[0].variantName.toString();
            setState(() {});
          }else{
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              children: [
                            ...
                            if(snapshot.hasData){
                              if(snapshot.data!.body.isEmpty){
                                ...
                                for (var i = 0; i < snapshot.data!.body.length; i++) {
                                  _vehicleVariant.add(snapshot.data!.body[i].variantName.toString());
                                }
                                DropdownMenuItem<String> varient(String i)=>DropdownMenuItem(
                                  value: i,
                                  child: Text(i),
                                );
                                return Stack(
                                  children: [
                                    DropdownButtonHideUnderline(
                                      child: DropdownButton<String>(
                                          items: _vehicleVariant.map(varient).toList(),
                                          value: selectedVariant,
                                          onChanged: (selctedValue){
                                            if(selctedValue == 'All Vehicles'){
                                              categoryController.selectedVariantId.value = '0';
                                              categoryController.selectedVariantName.value = 'All Vehicles';
                                              setState(() {});
                                              debugPrint(categoryController.selectedVariantName.value);
                                            }else{
                                              debugPrint(selctedValue);
                                              var index = _vehicleVariant.indexOf(selctedValue.toString());
                                              debugPrint(snapshot.data!.body[index-1].variantName.toString());
                                              debugPrint(snapshot.data!.body[index-1].variantId.toString());

                                              setState(() {
                                                categoryController.selectedVariantId.value = snapshot.data!.body[index-1].variantId.toString();
                                                categoryController.selectedVariantName.value = snapshot.data!.body[index-1].variantName.toString();
                                              });
                                              debugPrint("variant name " + categoryController.selectedVariantName.value.toString());
                                            }
                                          },
                                        // hint: Text('vehicle'),
                                        isExpanded: true,
                                    ...
                                    Container(
                                      child: Text(categoryController.selectedVariantName.value.toString(), maxLines: 1, overflow: TextOverflow.ellipsis),
                                      padding: const EdgeInsets.symmetric(vertical: 16),
                ...
                TabbarPage(
                  children: [ProductViewPage(token: box1!.get("data4"), userId: box1!.get("data3"),), ServicePage()],
                )
              ],
            ),
          ),
                    ...
                    child: Container(
                      ...
                      child: MySearchField(
                        ...
                        onTap: (value) {
                          ...
                          categoryController.vehicleTypeId.value = snapshot.data!.body[index].id.toString();
                          vehicleBrand.clear();
                          vehicleModel.clear();
            ...
            FutureBuilder<VechiletypeBrand?>(
              ...
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  ...
                  for (var i = 0; i < snapshot.data!.body.length; i++) {
                    _vehicleBrand.add(
                        snapshot.data!.body[i].brandName.toString());
                  }
                  return Padding(
                    padding:
                    const EdgeInsets.symmetric(horizontal: 30),
                    ...
                    child: Container(
                      ...
                      child: MySearchField(
                        ...
                        onTap: (value) {
                          ...
                          categoryController.vehicleVariantId.value = snapshot.data!.body[index].id.toString();
                          categoryController.selectedVariantId.value = snapshot.data!.body[index].id.toString();
                          categoryController.selectedVariantName.value = snapshot.data!.body[index].vehvarName.toString();
                          setState(() {});
                          debugPrint(categoryController.selectedVariantId.value);
                        },
                      ),
                    ),
                  );
                ...
                onPressed: (){
                  if(addVehicle.value == true){
                    service.postvechilebyuser(
                        ...
                        categoryid: categoryController.vehicleTypeId.value.toString(),
                        brandid: categoryController.vehicleBrandId.value.toString(),
                        variantid: categoryController.vehicleVariantId.value.toString(),
                        modelid: categoryController.vehicleModelId.value.toString(),
                        vehicleNumber: '').then((value) {
                          setState(() {});
                    });
                  }else{
                    // Get.back();
